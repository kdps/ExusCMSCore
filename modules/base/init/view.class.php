...
final class init_view {
	...
	function setModel() {
		...
		if (class_exists($this->modelObject)) {
			...
			$this->moduleHandler->setModel($modelObject);
		...
		if (class_exists($this->queryObject)) {
			...
			$this->moduleHandler->setQuery($queryObject);
		...
		if (class_exists($this->dtoObject)) {
			$dtoObject = new $this->dtoObject($this);
			$this->moduleHandler->setDTO($dtoObject);
	...
	function initializeModule($requestType = 'view') {
		...
		if (is_dir($this->moduleDirectory) && is_string($this->moduleID)) {
			...
			if (file_exists($ruleset) && $this->requestMethod != 'REQUEST') {
				...
				foreach ($_ruleset as $key => $val) {
					if (isset($_ruleset[$key])) {
						$vars = ($this->requestMethod === "GET") ? $this->base->get_params($key, 'string') : 
								($this->requestMethod === "POST" ? $this->base->post_params($key) : null);
			...
			if (file_exists($baseComponent)) {
				$baseClass = sprintf("%s/base.class.php", $this->moduleDirectory);
				
				if (file_exists($baseClass)) {
					$this->base->includeFile($baseClass, $this->moduleID);
							...
							if ($requestType == 'view') {
								if ($this->moduleID == 'board' && class_exists('board_model')) {
									...
									if (class_exists("board_query")) {
										$queryObject = new board_query($this);
										$this->moduleHandler->setQuery($queryObject);
									...
									if (class_exists("board_model")) {
										$modelObject = new board_model($this);
										$this->moduleHandler->setModel($modelObject);
									...
									$skin = $this->board->query->get_skin($this->module);
							
									$this->{$this->moduleID}->tpl_path = sprintf("%s/board/%s/skins", __MOD, $this->module);
									$this->{$this->moduleID}->skin_tpl_path = sprintf("%s/%s", $this->{$this->moduleID}->tpl_path, $skin);
									
									$this->{$this->moduleID}->tpl_url_path = sprintf("%s/board/%s/skins", __PATH, $this->module);
									$this->{$this->moduleID}->skin_tpl_url_path = sprintf("%s/%s", $this->{$this->moduleID}->tpl_url_path, $skin);
								} else {
									$this->setModel();
	...
	function processGET() {
		...
		if (isset($this->requestModuleID)) {
			if (preg_match("/^[a-zA-Z0-9\_\-]{1,50}+$/", $this->requestModuleID) && is_string($this->requestModuleID)) {
				...
				if ($this->base->isInstalled()) {
					$this->isModuleExists = $this->init->model->isModuleExits($this->requestModuleID);
					
					if ($this->isModuleExists === false) {
		...
		if (isset($this->requestModuleID) && is_string($this->requestModuleID) && preg_match("/^[a-zA-Z0-9\_\-]{1,50}+$/", $this->requestModuleID)) {
			...
			if (empty($this->skin)) {
				...
				if (is_array($this->layout)) {
					$this->skin = array_shift($this->layout);
				} else {
	...
	function init() {
		...
		// Specify default resource file.
		 $this->base->addJS(
			'head',
			"/common/assets/js/jquery/jquery-3.2.1.min.js",
			"/common/assets/js/jquery.migrate/jquery.migrate-1.2.1.js",
			"/common/assets/js/jquery-ui/jquery-ui.min.js",
			"/common/assets/js/jquery-jtemplates/jquery-jtemplates.js",
			"/common/assets/js/swfobject/swfobject.js",
			
			"/common/assets/js/cryptoJS/core.js",
			"/common/assets/js/cryptoJS/cipher-core.js",
			"/common/assets/js/cryptoJS/crypto-js.js",
			"/common/assets/js/cryptoJS/cryptoJS.min.js",
			"/common/assets/js/coreJS/dist/config.js",
			"/common/assets/js/coreJS/dist/variables.js",
			"/common/assets/js/coreJS/dist/coreJS.js",
			"/common/assets/js/coreJS/dist/extend.js",
			...
			"/common/assets/js/messenger/messenger-theme-flat.js",
			"/common/assets/js/messenger/messenger.option.js"
		);
		
		$this->base->addCSS(
			'head',
			"/common/assets/css/reset.css",
			"/common/assets/css/fontawesome.css",
			"/common/assets/css/messenger/messenger.css",
			"/common/assets/css/messenger/messenger-spinner.css",
			"/common/assets/css/messenger/messenger-theme-flat.css",
			"/common/assets/css/messenger/messenger-theme-future.css",
			"/common/assets/css/messenger/messenger-theme-ice.css",
			"/common/assets/css/messenger/messenger-theme-block.css",
			"/common/assets/css/messenger/messenger-theme-air.css"
		);
	...
	function callMethod($module, $oModule, $default_action, $private_action) {
		...
		if (((empty($_GET) && $this->requestMethod === "GET") || (empty($_POST) && $this->requestMethod === "POST"))) {
			if (isset($default_action) && method_exists($oModule, $default_action) && is_object($oModule)) {
			...
			if (isset($this->action) && method_exists($oModule, $this->action) && !in_array($this->action, $private_action) && is_object($oModule)) {
