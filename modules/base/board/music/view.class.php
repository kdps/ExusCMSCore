...
class board_view extends view_abstract implements viewInterface {
	...
	function dispBoardWrite() {
		if (!$this->hasGrant(false)) {
			return $this->modelObject->dispMemberLogin();
		}
		...
		$this->board->extra_var = $this->getDocumentExtraVars();//$this->queryObject->getExtraVar($this->getParam(__MODULEID));
		$this->board->srl = $this->getSrl();
	...
	function dispBoardModify() {
		...
		$this->board->extra_var = $this->getDocumentExtraVars();
	...
	function dispOriginContent() {
		...
		if ($this->board->item_popular == NULL) {
			...
			$this->board->file_list = $this->queryObject->getFileItemsArray($this->board->item_popular);
		}
		...
		$this->board->document = $this->queryObject->getDocumentItem($this->board->item_popular[0]['srl']);
	...
	function dispAlbumContent() {
		...
		$this->board->page_start = $this->getCurrentListCount();
		$this->board->album = $this->queryObject->getAlbumbysrl($this->board->related);
		$this->board->item_popular = $this->queryObject->getAlbumFilesAll($this->board->album);
	...
	protected function getChildCategory($subSrl, $toChildren = false) {
		...
		$this->board->module_id = $this->getModuleID();
		$subCategory = $this->queryObject->getSubCategoryList($this->board->module_id, $subSrl);
	...
	function dispBoardExtraSetup() {
		...
		$this->board->module_id = $this->getModuleID();
		$this->board->extralist = $this->queryObject->getExtraVar($this->board->module_id);
		$this->base->set('config', $this->board->config);
		$this->base->set('tab', $this->getTpl('setup_tab.php'));
	...
	function dispBoardPlaylist() {
		...
		if ($this->isLogged()) {
			...
			$this->board->result = $this->queryObject->getDocumentListInDocumentSrls(array_slice($playlist, $this->board->page_start, $this->board->config->list_count));
		}
		
		if (isset($this->board->result)) {
	...
	function dispBoardPopular() {
		...
		$this->board->page_start = $this->getCurrentListCount();
		$this->board->result = $this->queryObject->getPopularQueryByJoin($this->board->module_id, $this->board->popular_count, $this->board->page_start, $this->board->list_count);
	...
	function dispBoardAuthor() {
		$this->board->result = $this->queryObject->getAuthor();
	...
	function dispBoardOrigin() {
		...
		$this->board->document_count = $this->queryObject->getOriginAlbumCount();
		$this->board->page_start = $this->getCurrentListCount();
	...
	function dispBoardAlbum() {
		...
		$this->board->document_count = $this->queryObject->getAlbumCount();
		$this->board->page_start = $this->getCurrentListCount();
	...
	protected function getPagination() {
		...
		$this->board->page_count = $this->board->page_navigation->getLastPage();
	}
		...
		if (isset($this->board->srl)) {
			...
			if (!is_array($this->board->document)) {
				...
				if ($this->board->comment_listcount > 0) {
					...
					$this->board->comment_navigation = $this->modelObject->getPageArray($this->board->comment_count_rel, $this->board->cpage);
			...
			if ($this->board->document['tag']) {
				...
				$this->board->relatedTagList->pagenation = $this->modelObject->getPageArray($this->board->relatedTagList->page_count, $this->board->relatedTagList->current_page);
	...
	protected function getCurrentPage():int {
		...
		if ($this->board->config->bd_query !== "LIMIT") {
			...
			$targetDocumentCount = $this->queryObject->getCountInTargetDocument($this->board->document['module'], $this->board->document['srl_bd'])->data();
		}
	...
	function dispBoardContent() {	
		...
		//장르
			...
			if (!isset($this->board->config->search_type)) {
				$this->board->config->search_type = 'Y';
			}
			...
			if (($this->board->config->list_view_on | 1) || !isset($this->board->srl)) {
				...
				if ($this->board->config->bd_query=="LIMIT") {
					...
					$this->board->result = array_reverse($this->queryObject->getDocumentList($this->getParam(__MODULEID), $this->board->page_start, $this->board->page_end));
				}
