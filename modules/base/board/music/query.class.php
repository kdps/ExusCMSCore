		...
		function UpdateBitrate($srl, $bitrate):bool
 {
	...
	/* Document List */
		...
		function getDocumentListLIMIT($module, $page_start, $board_count) {
			...
			], 'def_document_music.*, (SELECT count(b.srl) FROM def_file b WHERE b.target = def_document_music.file_sequence) as `file_count`, (SELECT count(c.comment_srl) FROM def_comment c WHERE c.document_srl = def_document_music.srl_bd) as `comment_count`, (SELECT name FROM def_category d WHERE d.category_srl = def_document_music.category_srl) as `category_caption`
			', 'all'));
		}
		...
		/**
		 * upload document genre original
		 *
		 * @param string $artist
		 * @param string $module
		 * @param int $srl
