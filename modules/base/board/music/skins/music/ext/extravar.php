...
<div class="extra_vars" style="display: block;<?php echo $this->board->config->tmp_hide_extravars==1 ? 'display:none':''?>">
	<table id="extravars" class="bd_tb">
		<tbody>
			<?php foreach($this->board->extra_vars as $key=>$val): ?>
				<tr class="bg1">
					<th style="background-color:#fbfbfb" scope="row"><?php echo $this->board->query->getExtraVarTypebyName($val->getName(), 'title', $this->board->module_id); ?></th>
