let highlightRunner = function (target) {
	try {
		let n = decodeURIComponent(target).replace(/.*#cmt_/, "");
		if (n) {
...
function addDispPrefix() {
	if (glob_srl !== null) {
		try {
			let target = $('div[data-srl=' + glob_srl + ']');
			if (target.length > 0) {
		...
		try {
			let target_focus_srl = $.core.URL.getParam('srl',window.location.href);
			if (target_focus_srl) {
...
function lazyloadRegsitry() {
    ...
    let pictureObserver = lozad('.lozad-picture', {
	    threshold: 1
    });
...
function popstateEvent(_this, event) {
	let target_url = event.target.location.href;
	let target_srl = $.core.URL.getParam('srl', target_url);
	let target_page = $.core.URL.getParam('page', target_url);
	
	if (!target_url || target_url.indexOf('#') > -1) return;
	
	reRegistryDocument();
	
	let url = $.core.URL.setParam('act', '', target_url);
...
function hyperlinkBookmarkEvent(_this, event) {
	let isSameURL = ($.core.Str.removeTagHash(_this.href) === $.core.Str.removeTagHash(location.href)) && 
	...
	if (isSameURL) {
		if (_this.hash) {
			...
			let $target = $(_this.hash), targetOffset;
...
function loadDoc(args) {
	let hasLength = readObject.length > 0;
	...
	if (hasLength) {
		...
		$("html, body").animate(
			{scrollTop: $('.list_area').position().top},
			{duration: 300,
				specialEasing: 
				{width: 'linear',height: 'easeInBounce'}
			}
		);
	}
}
...
function formSubmitEvent(event) {
	let bdsearch = $(event.target).hasClass(searchForm);
...
function loadTargetDocument(_this, url, event) {
	let hasItem = readObject.length >0 || $('.related_doc').length > 0;
	...
	if (hasItem) {
		...
		if (core_flower.bdstyle == 'flat') {
			let srl = $.core.URL.getParam('srl',url);
			glob_srl = srl;
			
			$('.bd_lst li div').removeClass(viewDocumentPrefix);
			$('.bd_lst a').filter(function() {
				let regex = 'srl=' + srl + '';
				let regexr = new RegExp(regex, 'g');
				
				let rewrite_regex = '^/([0-9]+)/([a-zA-Z0-9_]+)$';
				let rewrite_regexr = new RegExp(regex, 'g');
				
				let link = $(this).attr('href') || this.href;
				if (!link) return;
				let isMatch = link.match(regexr);
				if (isMatch) {
			...
			$('.bd_tb tbody tr').filter(function() {
				...
				let isMatch = ($(this).attr('href').match(regexr) || this.href.match(rewrite_regexr));
				if (isMatch) {
...
function hyperlinkEvent(_this, event) {
	let tag = event.currentTarget.tagName.toUpperCase();
	let url = event.currentTarget.href;
	let navi_page = $(event.target).hasClass("navi_page");
	let comment_navi = $(event.target).hasClass("comment_navi");
	let isAjaxDocument = $(event.target).hasClass(ajaxDocument);
	let isTargetLink = (tag === 'A' && !url.match(url));
