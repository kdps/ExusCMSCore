...
(function(core) {
	core.Request.addAjaxCallback('completeLyrics', function (_) {
		_.type === "success" && core.Lyrics.isValidLyricsData(_.html) ? 
			$(".lyrics_display").html(_.html) && $(".lyrics_display").children().hide() && core.Lyrics.setEventListener() : 
			$(".lyrics_display").html(_.html), $(".lyrics_display_expand").html(_.html);
							
		if ("success" === _.type && core.Lyrics.isValidLyricsData(_.html)) {
			...
			$("#lyrics_display_inner").append("<div id=resizable></div>");
			
			core.Lyrics.setResizable();
		}
	});
	...
	core.Request.addAjaxCallback('autocomplete', function (args) {
		...
		if (core.JSON.isJSON(args)) {
			...
			$('#autoload_related_list').setTemplateURL("modules/base/board/music/skins/music/template/related_tag_autocomplete.tpl").processTemplate(tag);
		} else {
	...
	core.Request.addAjaxCallback('complteLoadReleatedList', function (args) {
		...
		if (core.JSON.isJSON(args)) {
			...
			var tag = args.tag_list;
			var navi = args.navigator;
			$('#prev_nav').setTemplateURL("modules/base/board/music/skins/music/template/related_btn.tpl").processTemplate(prev_navi);
			$('#next_nav').setTemplateURL("modules/base/board/music/skins/music/template/related_btn.tpl").processTemplate(next_navi);
			$('#related_list').setTemplateURL("modules/base/board/music/skins/music/template/related_tag.tpl").processTemplate(tag);
			$('#related_nav_page').setTemplateURL("modules/base/board/music/skins/music/template/related_navi.tpl").processTemplate(navi);
		} else {
