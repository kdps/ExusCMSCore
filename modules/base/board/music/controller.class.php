<?php
	...
	class board_controller extends controller_abstract implements controllerInterface 
	{
		...
		function insertDocument() 
		{
			...
			$this->post_data->title = $this->getTitle();
			...
			$this->board->extraKeyList = $this->getExtraKeyList();
			...
			{
				$this->post_data->lastId = $this->getNextSequence();
				
				$this->insertDocumentItem();
				
				// Get a last srl
				$this->post_data->lastID = $this->pdo->lastInsertId('srl');
		...
		function procAddPlaylist() 
		{
			...
			else 
			{
				...
				if (is_array($userExtraVars['playlist'])) 
				{
					if (in_array($target_srl, $userExtraVars['playlist'])) 
					{
		...
		function procBoardRelatedList() 
		{
			...
			if (isset($this->post_data->pos) && isset($this->post_data->tag)) 
			{
				...
				$oTagDocument = $this->board->query->getTagRelatedDocumentSrl($this->post_data->module_id, $this->post_data->pos, $this->list_count,$this->post_data->tag);
