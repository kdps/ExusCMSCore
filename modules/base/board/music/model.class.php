...
class board_model extends board {
	...
	function getExtraVarsHTML($val) {
		$type = $this->board->query->getExtraVarTypebyName($val->getName(), 'type', $this->board->module_id);
		$var = strip_tags($val->getValue());
		
		switch($type) {
			case 'textarea':
				$output = "<span style=\"white-space: pre-wrap;\">{$var}</span>";
				break;
			case 'color':
				$output = "<span style=\"background-color:{$var}\">{$var}</span>";
				break;
			case 'file':
				$filename = $val->getAttachedFileName();
				if (!$filename)
				{
					break;
				}
				...
				switch($fileExtension)
				{
					case "jpg":
						...
						break;
					case "mp3":
						$output = "<audio controls src=\"{$fileSrl}\">";
						break;
					case "mp4":
						$output = "<video src=\"{$fileSrl}\">";
						$output = "<video width=\"320\" height=\"240\" controls><source src=\"{$fileSrl}\" type=\"video/mp4\">Your browser does not support the video tag.</video>";
						break;
					default:
