...
class files_model extends files 
{
	...
	function getFile($module, $fileSrl) 
	{
		return db::Query('SELECT','def_file',
		[
			['AND', 'module', '=', ':module', $module],
			...
			['ORDER', 'srl', 'desc'],
			['LIMIT', ':pgx', 0, ':pgy', 1]
		],'module, origin, target, files, srl', 'all', '', 'object');
	}
	 ...
	 * @param string $files
	 */
	function insertExtraFileList($target_srl, $extrakey, $files, $origin) 
	{
		$sth = $this->pdo->prepare("INSERT INTO def_extrafile (target_srl, extrakey, files, origin) VALUES (:target_srl, :extrakey, :files, :origin)");
		$sth->bindParam(':target_srl', $target_srl, PDO::PARAM_INT);
		$sth->bindParam(':extrakey', $extrakey, PDO::PARAM_STR);
		$sth->bindParam(':files', $files, PDO::PARAM_STR);
		$sth->bindParam(':origin', $origin, PDO::PARAM_STR);
		$sth->execute();
	}
	
	function getFileSequenceBySrl($srl) 
	{
		$sth = $this->pdo->prepare("SELECT member_srl FROM def_file_sequence WHERE srl = (SELECT target FROM def_file WHERE srl = :srl)");
	...
	function getExtraFileByKey($srl, $key) 
	{
		return db::Query('SELECT','def_extrafile',
		[
	...
	function deleteExtraFile($srl, $key) 
	{
		return db::Query('DELETE','def_extrafile',
		[
			['AND', 'target_srl', '=', ':args1', $srl],
			['', 'extrakey', '=', ':args2', $key]
		],'', 'boolean');
	...
	function updateExtraFile($origin, $files, $target_srl, $extrakey) 
	{
		$sql = "UPDATE def_extrafile SET origin = :args1, files = :args2 WHERE target_srl = :args3 AND extrakey = :args4";
		$sth = db::Compile($sql);
		db::BindParams($sth, [
			"args1" => $origin,
			"args2" => $files,
