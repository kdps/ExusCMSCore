...
class files_view extends files 
{
	...
	function getCoverStream() 
	{
		...
		if (substr($_SERVER['HTTP_ACCEPT'],0,5)!="image" || $_SERVER['HTTP_CACHE_CONTROL']=='no-cache') 
		{
			...
			$this->files->file_obj = $this->modelObject->getFilebySrl($this->files->audio_srl);
	...
	function AlbumDownload() 
	{
		...
		$this->files->album = $this->modelObject->getOriginAlbumbysrl($this->files->target);
		$this->files->item_popular = $this->modelObject->getOriginAlbumFilesAll($this->files->album);
		$this->files->file_list = $this->modelObject->getFileItemsArray(($this->files->item_popular));
	...
	function FileDownload() 
	{
		...
		$filename = $this->modelObject->getAttachOrigin($this->files->download, $this->files->md5);
		$down_count = $this->modelObject->getFileDownCount($this->files->download, $this->files->md5);
		...
		foreach ($filename as $value) 
		{
			...
			if (preg_match('/\.(jpg|gif|png|mp3|mp4|mkv|avi|zip|7z|rar|gz)(?:[\?\#].*)?$/i', $uploaded_filename, $matches)) 
			{
				...
				$this->modelObject->UpdateDownCount($down_count + 1, $this->files->download, $this->files->md5);
				exit();
			}
		}
	}
